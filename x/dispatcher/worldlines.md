...
- **Merkle DAG Architecture:**  
  - **Events in Leaves:**  
    Events are stored in leaves in the DAG. 
    Importantly, these leaves do not include the hash of the previous event to avoid redundancy.

- **Internal Nodes are Edit Logs:**  
  - Internal nodes log the promise messages (edit operations) that modify one or more worldlines. 
  - They store the hashes of their children.
  - A message is a compound promise.  The compound promise is made up
    of multiple claims, each expressed as a CWT claim.  
  - Each claim is an edit operation on the merkle DAG that represents a worldline.  
